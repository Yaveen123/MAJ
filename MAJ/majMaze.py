...
def printMaze(maze):
    ...
    for i in range(0, height):
        for j in range(0, width):
            if (maze[i][j] == 'u'):
                mazeStr = mazeStr + Fore.WHITE + str(maze[i][j]) 
            elif (maze[i][j] == ' '):
                mazeStr = mazeStr + Fore.GREEN + str(maze[i][j])
            else:
                mazeStr = mazeStr + Fore.RED + str(maze[i][j])
...
def runMaze(w, h, playerChar):
    ...
    maze[starting_height][starting_width] = playerChar
    # Print final maze
    return (maze, starting_width, starting_height)
